...
export const SettingsScreen: Screen = (app) => {
  ...
  app.root.append(h("main", { class: "page" },
    ...
    select("inputMode", "Frame input", [["video", "Live video (default)"], ["canvas", "Copied frame (try if the face isn't found)"]]),
    select("trackerCanvas", "Tracker canvas", [["auto", "Automatic (off on iPhone/iPad)"], ["on", "App-supplied (detects graphics resets)"], ["off", "Tracker's own"]]),
    select("cameraResolution", "Camera resolution", [[720, "720p (better eye detail)"], [480, "480p (lighter)"]]),
    h("p", { class: "fineprint" }, "Processor, canvas and resolution changes apply from the next session. Diagnostics can find the best combination for this device."),
    h("button", { onclick: () => app.go("diagnostics") }, "Open diagnostics"),
    h("h2", {}, "Developer"),
